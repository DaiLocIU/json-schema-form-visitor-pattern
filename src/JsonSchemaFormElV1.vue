...
watch(() => props.modelValue, (v) => { state.value = v }, { immediate: true })
watch(() => state.value, (v) => {
...
function getByPointer(root: any, pointer: string) {
  if (pointer?.startsWith('#/')) {
    return pointer.slice(2).split('/').reduce((acc: any, seg: string) => acc?.[seg], root)
  }
  return undefined
}
...
function resolveSchema(def: any, seen = new Set<any>()): any {
  ...
  if (Array.isArray(def.allOf)) {
    const merged = def.allOf.map((d: any) => resolveSchema(d, seen))
  ...
  if (def.prefixItems) out.prefixItems = def.prefixItems.map((d: any) => resolveSchema(d, seen))
  if (def.properties) {
    out.properties = {}
...
const RENDER: Record<string, Handler> = {
  string: (sch, ctx) => {
    ...
    if (Array.isArray(sch.enum)) {
      return h('div', { class: 'field' }, [
        ...
        h('select', {
          value: ctx.value ?? '',
          onChange: (e: Event) => ctx.setValue((e.target as HTMLSelectElement).value)
        }, [
          h('option', { value: '' }, '— select —'),
          ...sch.enum.map((opt: any) => h('option', { value: String(opt) }, String(opt)))
        ]),
    ...
    const inputType =
      sch.format === 'date' ? 'date'
      : sch.format === 'time' ? 'time'
      : sch.format === 'date-time' ? 'datetime-local'
      : 'text'
    return h('div', { class: 'field' }, [
      h('label', sch.title ?? ctx.path.at(-1) ?? 'string'),
      h('input', {
        ...
        onInput: (e: Event) => ctx.setValue((e.target as HTMLInputElement).value),
    ...
    h('div', { class: 'field' }, [
      h('label', [
        h('input', {
          type: 'checkbox',
          checked: !!ctx.value,
          onInput: (e: Event) => ctx.setValue((e.target as HTMLInputElement).checked)
        }),
  ...
  object: (sch, ctx) => {
    ...
    for (const [k, child] of Object.entries<any>(props)) {
      ...
      children.push(renderNode(childSchema, {
        schema: childSchema,
        path: childPath,
        value: childVal,
      ...
      children.push(
        h('div', { class: 'row' }, [
          h('input', {
            ...
            onInput: (e: Event) => (newKey = (e.target as HTMLInputElement).value)
          }),
          h('button', {
            class: 'btn',
            onClick: () => {
              ...
              else if (template.type === 'number' || template.type === 'integer') init = 0
    ...
    return h('fieldset', { class: 'group' }, [
      (sch.title ?? String(ctx.path.at(-1) ?? '')) ? h('legend', sch.title ?? String(ctx.path.at(-1))) : null,
      ...children
    ])
  },
      ...
      h('div', { class: 'prop-card' }, [
        h('div', { class: 'prop-head' }, [
          ...
          h('button', { class: 'link danger', onClick: () => {
            const cloned = [...arr]; cloned.splice(i, 1); ctx.setValue(cloned)
          }}, 'Remove')
        ]),
        renderNode(itemsSchema, {
          ...
          value: arr[i],
          setValue: (v) => {
            const cloned = [...arr]; cloned[i] = v; ctx.setValue(cloned)
          }
        })
      ])
    )
  ...
  unknown: (_sch, ctx) => h('div', { class: 'field dim' }, `No renderer for type`)
}
...
function renderComposite(s: any, ctx: Ctx): VNode | null {
  ...
  const hasSelection = typeof idx === 'number'

  const titles = s.oneOf.map((opt: any, i: number) => {
    if (opt.title) return opt.title
    if (opt.type) return String(opt.type)
    if (Array.isArray(opt.enum)) return `enum: ${opt.enum.join(', ')}`
    if (opt.$ref) return String(opt.$ref).replace('#/$defs/', '').replace('#/definitions/', '')
    return `Option ${i + 1}`
  })
  ...
  return h('div', { class: 'oneof' }, [
    ...
    h('select', {
      ...
      h('option', { value: '' }, '— select —'),
      ...titles.map((t: string, i: number) => h('option', { value: String(i) }, t))
    ]),

    hasSelection ? renderNode(s.oneOf[idx as number], ctx) : null
  ])
}
  ...
  //   const titles = s.anyOf.map((o: any, i: number) => o.title ?? `Option ${i + 1}`)
  ...
  //     // merge init
  //     let merged: any = {}
  //     for (const idx of selected) merged = deepMerge(merged, initBySchema(s.anyOf[idx]))
  //     ctx.setValue(merged)
  //   }
  ...
  //         onInput: (e: Event) => toggle(i, (e.target as HTMLInputElement).checked)
  //       }),
  //       ` ${t}`
  //     ])
  //   )
  ...
  const selKey = `anyOf:${keyOf(ctx.path)}`
  const idx = branchState[selKey]  // do NOT default to 0
  const hasSelection = typeof idx === 'number'

  const titles = s.anyOf.map((opt: any, i: number) => {
    if (opt.title) return opt.title
    if (opt.type) return String(opt.type)
    if (Array.isArray(opt.enum)) return `enum: ${opt.enum.join(', ')}`
    if (opt.$ref) return String(opt.$ref).replace('#/$defs/', '').replace('#/definitions/', '')
  ...
  return h('div', { class: 'anyof' }, [
    h('label', s.title ?? String(ctx.path.at(-1) ?? 'anyOf')),
    h('select', {
      value: hasSelection ? String(idx) : '',
      onChange: (e: Event) => {
        const v = (e.target as HTMLSelectElement).value
        branchState[selKey] = v === '' ? undefined : Number(v)
        // IMPORTANT: do NOT call ctx.setValue() here
      }
    }, [
      h('option', { value: '' }, '— select —'),
      ...titles.map((t: string, i: number) => h('option', { value: String(i) }, t))
    ...
    // Only render the chosen branch AFTER user selects one
    hasSelection ? renderNode(s.anyOf[idx as number], ctx) : null
  ])
}
 ...
 *  Render node đệ quy
 *  ========================= */
function renderNode(schRaw: any, ctxBase: Ctx, label?: string, required = false): VNode {
...
.field select {
  padding: 8px 10px; border: 1px solid #ddd; border-radius: 8px; width: 100%;
}
.group { border: 1px solid #eee; border-radius: 12px; padding: 12px; margin: 12px 0; }
.group > legend { font-weight: 700; padding: 0 6px; }
.array .prop-card, .prop-card { border: 1px solid #eee; border-radius: 10px; margin: 8px 0; padding: 10px; }
.prop-head { display: flex; justify-content: space-between; margin-bottom: 8px; }
.hint { color: #666; font-size: 12px; }
.req { color: #b3261e; margin-left: 4px; }
.btn { padding: 6px 10px; border: 1px solid #111; border-radius: 8px; background: #111; color: #fff; cursor: pointer; }
.link { border: 0; background: transparent; cursor: pointer; color: #1867C0; }
.link.danger { color: #B00020; }
.row { display: flex; gap: 8px; align-items: center; }
.row.wrap { flex-wrap: wrap }
.checkbox { margin-right: 12px; }
.oneof, .anyof { border: 1px dashed #ddd; border-radius: 10px; padding: 10px; margin: 8px 0; }
.lbl-inline { margin-right: 6px; }
</style>

<template>
  <main class="wrap">
    <h1>200 Hearts — Bad (write→read→write) vs Best (all reads → all writes)</h1>
    ...
    <div ref="stageRef" class="stage" aria-label="love scene">
      <div
        v-for="i in hearts"
        :key="i"
        class="love"
...
// ---------- config ----------
const COUNT = 2000  // Reduced from 1000 for better FPS
...
const hearts = ref(Array.from({ length: COUNT }, (_, i) => i + 1))
const heartEls = ref(Array(COUNT).fill(null))
const mode = ref('bad') // 'bad' | 'best'
const scheduler = ref('fdom') // 'fdom' | 'raf'
let timer = null

// ---------- helpers ----------
function setHeartRef(idx) {
  return (el) => { if (el) heartEls.value[idx] = el }
}
...
async function placeRandom() {
  ...
  for (const el of heartEls.value) {
    if (!el) continue
...
function stepBest() {
  ...
  if (scheduler.value === 'raf') {
    console.log('useeee raf love')
    // RAF-based two-phase batching (reads → writes)
    const targets = new Array(heartEls.value.length)
    requestAnimationFrame(() => {
      const stageRect = stage.getBoundingClientRect()
      for (let i = 0; i < heartEls.value.length; i++) {
        const el = heartEls.value[i]
        if (!el) continue
        const oldRect = el.getBoundingClientRect()
        const currX = oldRect.left - stageRect.left
        const currY = oldRect.top  - stageRect.top
     ...
     console.log('useeee fasterdom love')
...
.love {
  ...
  color: #ff6b9d; font-size: 12px; pointer-events: none;
